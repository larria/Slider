	...
	init.prototype = {
		...
		_initDoms : function (opts) {
			var that = this,
				...
				flStr, itemsStr, inner = wrap.innerHTML, initStr,
				aniWrap, items;
		...
		_initStates : function (opts) {
			...
			onTransitionEnd = function() {
				var c = that.conf, d = that.doms, s = that.states,
		...
		_initEvents : function (opts) {
			...
			if(s.supportTouch){
				this._addEvent(wrap, 'touchstart', function(e){that._touchstart(e);});
				this._addEvent(wrap, 'touchmove', function(e){that._touchmove(e);});
				this._addEvent(wrap, 'touchend', function(e){that._touchend(e);});
			}
		},
		_launch : function () {
		...
		_moveToEle : function (to, isOffset, onMoveEnd) {
			var that = this,
				...
				aniWrap = d.aniWrap, items = d.items, toItem = items[toIdx],
				css3Prop = this._css3Prop(), toPos, toPosStr;
		...
		_slideToCss3 : function (fIndex) {
			var that = this,
				c = this.conf, d = this.doms, s = this.states,
				items = d.items, aniWrap = d.aniWrap,
				css3Prop = this._css3Prop(), toPos, toPosStr;
		...
		slideTo : function (i) {
			...
			/*if(c.isLoop && Math.abs(fnlIndex - curIdx) > total / 2){
				fnlIndex = fnlIndex < curIdx ? fnlIndex + total / 2 : fnlIndex - total / 2;
			}*/
			if(fnlIndex == curIdx){
				return;
			}
